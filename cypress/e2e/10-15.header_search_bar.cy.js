...
describe('12 - Busque na API de comidas caso a pessoa esteja na página de comidas, e na API de bebidas caso esteja na de bebidas', () => {
  ...
  it('Verifica a cobertura de 45% do Componente SearchBar', () => {
    cy.task('getCoverage', getId()).its('SearchBar.functions.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.task('getCoverage', getId()).its('SearchBar.lines.pct', { timeout: 0 }).should('be.gte', 45.00);
    cy.task('getCoverage', getId()).its('SearchBar.branches.pct', { timeout: 0 }).should('be.gte', 45.00);
  });
});
...
describe.only('15 - Exiba um `alert` caso nenhuma receita seja encontrada', () => {
  ...
  it('Verifica a cobertura de 90% do Componente SearchBar', () => {
    cy.task('getCoverage', getId()).its('SearchBar.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('SearchBar.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('SearchBar.branches.pct', { timeout: 0 }).should('be.gte', 90.00);
  });
});
